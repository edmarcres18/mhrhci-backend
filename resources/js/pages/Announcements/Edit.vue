...
const breadcrumbs = [
  ...
  { title: 'Edit', href: `/announcements/${props.announcement.id}/edit` },
];
...
function submit() {
  ...
  form
    ...
    .post(`/announcements/${props.announcement.id}`, {
      preserveScroll: true,
      onSuccess: () => {
        showToast('success', 'Announcement updated successfully!');
      },
      onError: () => {
        showToast('error', 'Failed to update announcement. Please check the form.');
      },
    });
}
...
<template>
  <Head :title="`Edit: ${props.announcement.title}`" />
