  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="mx-auto w-full max-w-7xl p-4 sm:p-6">
      ...
      <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div class="flex-1">
          <h1 class="text-2xl font-bold text-neutral-900 dark:text-white sm:text-3xl">{{ props.blog.title }}</h1>
          <p class="mt-2 flex items-center gap-1 text-sm text-neutral-500 dark:text-neutral-400">
            ...
            Created {{ formatDate(props.blog.created_at) }}
          </p>
        </div>
      ...
      <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6 lg:col-span-2">
          ...
          <div class="overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
            <div v-if="props.blog.images?.length" class="space-y-4 p-4">
              <!-- Main Image -->
              <div 
              ...
              >
                ...
                <div class="absolute left-4 top-4 rounded-full bg-black/70 px-3 py-1 text-xs font-semibold text-white backdrop-blur-sm">
                  {{ selectedImageIndex + 1 }} / {{ props.blog.images.length }}
                </div>
              </div>
              
              <!-- Thumbnails -->
              <div v-if="props.blog.images.length > 1" class="grid grid-cols-4 gap-2 sm:grid-cols-5 md:grid-cols-6">
                <button
                ...
                >
                  <img 
                    :src="imageUrl(img)" 
                    :alt="`${props.blog.title} - Image ${i + 1}`"
            ...
            <div v-else class="flex flex-col items-center justify-center p-16 text-center">
              ...
              <p class="mt-1 text-sm text-neutral-500">This blog doesn't have any images yet.</p>
            </div>
          </div>
          ...
          <div class="rounded-xl border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
            <div class="mb-4 flex items-center gap-2">
              ...
              <h2 class="text-lg font-bold text-neutral-900 dark:text-white">Content</h2>
            </div>
            <div v-if="props.blog.content" class="prose prose-sm max-w-none dark:prose-invert">
            ...
            <div v-else class="rounded-lg border-2 border-dashed border-neutral-200 p-8 text-center dark:border-neutral-800">
              ...
              <p class="mt-2 text-sm text-neutral-500">No content provided.</p>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          ...
          <div class="rounded-xl border border-neutral-200 bg-gradient-to-br from-neutral-50 to-white p-6 shadow-sm dark:border-neutral-800 dark:from-neutral-950 dark:to-neutral-900">
            <div class="mb-4 flex items-center gap-2">
              ...
              <h2 class="text-lg font-bold text-neutral-900 dark:text-white">Blog Info</h2>
            </div>
            <div class="space-y-3">
              <div class="flex items-center justify-between rounded-lg border border-neutral-200 bg-white p-3 dark:border-neutral-800 dark:bg-neutral-950">
                <span class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Blog ID</span>
                <span class="font-mono text-sm font-semibold text-neutral-900 dark:text-white">#{{ props.blog.id }}</span>
              </div>
              <div class="rounded-lg border border-neutral-200 bg-white p-3 dark:border-neutral-800 dark:bg-neutral-950">
                ...
                <div class="text-sm font-semibold text-neutral-900 dark:text-white">{{ formatDate(props.blog.created_at) }}</div>
              </div>
              <div class="flex items-center justify-between rounded-lg border border-neutral-200 bg-white p-3 dark:border-neutral-800 dark:bg-neutral-950">
    ...
    <Dialog v-model:open="confirmOpen">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogTitle class="text-center text-xl">Delete Blog?</DialogTitle>
          <DialogDescription class="text-center">
            This action cannot be undone. This will permanently delete
            <span class="font-semibold text-neutral-900 dark:text-white">{{ props.blog.title }}</span> and remove all its images.
          </DialogDescription>
        </DialogHeader>
    ...
    <Dialog v-model:open="isImageModalOpen">
      <DialogContent class="max-w-5xl border-0 bg-black/95 p-0">
        ...
        <button 
          v-if="props.blog.images && selectedImageIndex < props.blog.images.length - 1"
          @click="nextImage" 
        ...
        <div class="relative aspect-video w-full">
          <img :src="selectedImage" :alt="props.blog.title" class="h-full w-full object-contain" />
          <div class="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-white/20 px-4 py-2 text-sm font-semibold text-white backdrop-blur-sm">
            {{ selectedImageIndex + 1 }} / {{ props.blog.images?.length || 0 }}
          </div>
        </div>
      </DialogContent>
    </Dialog>
