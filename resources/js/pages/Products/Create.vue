...
const form = useForm({
  name: '',
  product_type: props.productTypes[0]?.value || 'medical_supplies',
  principal_id: props.principals[0]?.id ?? null,
  description: '',
  features: [] as string[],
  images: [] as File[],
...
function submit() {
  ...
  form.post('/products', {
    forceFormData: true,
    onSuccess: () => {
      form.reset('name', 'description', 'features', 'images');
      showToast('success', 'Product created successfully!');
    },
    onError: () => {
      showToast('error', 'Failed to create product. Please check the form.');
    },
  });
}
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="mx-auto w-full max-w-3xl p-4">
      ...
      <form @submit.prevent="submit" class="space-y-6 rounded-xl border border-sidebar-border/70 bg-white p-6 shadow-sm dark:border-sidebar-border dark:bg-neutral-950 sm:p-8">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <label class="block text-sm font-semibold text-neutral-700 dark:text-neutral-200">Product Images</label>
